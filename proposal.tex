...
\begin{abstract}
    ...
    human CD34$^{+}$ cells without TET2 mutants. A visible neural network (VNN) on
    McClatchy's Tet2 $\times$ IL-1 HSPC gene scores meets $p=0.0001$ for HMP shunt in
    mouse, matching Kim's tracing. Human TET2 $\times$ LPS encodings meet glycolysis,
    Complex~I, Complex~II, and HMP shunt. We hypothesize that TET2 loss changes how
    IL-1R allocates glucose carbon and redox (PPP / UDP-GlcNAc / OGT). Polar LC-MS of
    cells and medium on the same four arms, a clone knockout plus IL-1 added, tests that
    allocation. Glycolysis and OXPHOS are human-LPS and Ni{\~n}o main effects. Lactate,
    succinate, itaconate, and $\alpha$-KG are secondary or negative reads on the mouse
    IL-1 2$\times$2.
\end{abstract}
...
The primary polar panel follows the mouse HMP-shunt encoding and Kim's tracing:
